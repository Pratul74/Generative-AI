...
prompt_template = ChatPromptTemplate.from_messages([
    (
        "system",
        """
You are an expert smartphone specification extractor.

Extract every specification mentioned in the input.

Rules:
- Never guess missing values.
- Use "Not specified" when unavailable.
- Preserve all RAM, storage variants, colors, AI features, camera features, etc.
"""
    ),
    ("human", "{phone_description}")
])
...
st.title("📱 Smartphone Specification Extractor")

des = st.text_area(
    "Paste your phone description:",
    height=250
)
...
if st.button("Extract Specifications"):
    if des.strip():
        ...
        st.warning("Please paste a phone description.")
